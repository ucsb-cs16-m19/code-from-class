...
int main() {
    ...
    // because we're providing those 5
    ...
    for (double item : scores) {
        cout << item << endl; // item will be 100 in the first iteration, 90 in the second iteration, etc.
    }
    ...
    // sizeof(scores) only works here because we told the compiler how big the array is
    // (the size of the array was known at compile-time)
    cout << "Size of scores: " << sizeof(scores) << endl;
    cout << "Size of a double: " << sizeof(double) << endl;

    cout << "Average midterm score is : " << 
            sum/(sizeof(scores)/sizeof(double)) << endl;
